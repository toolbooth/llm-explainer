  ...
  return (
    ...
    >
      ...
      <nav className="cl-beats cl-noprint" aria-label="Lesson beats">
        {beatNav.map((b) => (
          <a href={`#/classroom/m1/${b.id}`} key={b.id} onClick={(e) => jump(e, b.id)}>
            {b.label}
          </a>
        ))}
      </nav>
...
function stepNum(htmlLang: string, n: number): string {
 ...
 */
function jump(e: MouseEvent<HTMLAnchorElement>, id: string): void {
