...
- **First interactive paint under 3 s on a 4 GB Chromebook over school Wi-Fi; weights (7.5 MB) and tokenizer files (~2 MB) fetched once, cached with long-lived headers; the page must work with 30 students fetching simultaneously** (~11 MB × 30 ≈ 330 MB burst, re-baselined 2026-09-15 with the measured table below — not the 225 MB the first draft computed from weights alone; acceptable on a school link, but test it, and note the unplugged opener staggers the loads for free).
- **Total first-visit page weight budget ≤ 11 MB on disk / ~8.5 MB on the wire, itemized** (re-baselined 2026-09-15, r3; the first draft's "≤ 10 MB including weights" silently omitted the tokenizer files and the JS bundle, then left the bundle "~0.5 MB" it never measured):
  ...
  **Re-baseline rationale (2026-09-15).** The built bundle measured 1.13 MB at phase 3, 1.29 MB after the phase-4 accessibility/service-worker commits, and 1.38 MB with the embed kit and evidence surface — the "~0.5 MB" line was an unmeasured guess, and the alternative it named (replacing transformers.js with a self-contained BPE tokenizer, REVIEW-CLASSROOM-3 open question 1) is declined: a tokenizer rewrite risks drift against the engine verified token-exact against Hugging Face `transformers`, to save ~0.16 MB of wire. The honest budget is what the build produces: **≤ 11 MB on disk, ~8.5 MB on the wire, first visit, everything included** — and the 30-client arithmetic above uses 11 MB. The old 10 MB figure survives in exactly one narrower form, with its own test (`test/tokenizer-locality.test.ts`): **weights + tokenizer must stay under 10 MB**, so the model itself can never eat the bundle's headroom.
...
| 2026-09-15 | r3 | §6.1 page-weight budget re-baselined against the built site: the "to measure at build" bundle line replaced by the measured table (app 0.82 MB + transformers.js 0.56 MB + tokenizer 2.11 MB + weights 7.50 MB = 10.99 MB on disk, ~8.5 MB on the wire, from `npm run build` 2026-09-15), total budget restated as ≤ 11 MB disk / ~8.5 MB wire, the 30-client burst recomputed at ~330 MB, and the "replace transformers.js with a self-contained BPE" alternative declined with reasons — closes REVIEW-CLASSROOM-3 open question 1. The weights + tokenizer ≤ 10 MB line keeps its test (`test/tokenizer-locality.test.ts`). Cross-referenced in the tech-check page (EN + 中文). | A budget the build has never met is not a budget; the measured table is the honest baseline, and the tokenizer-rewrite escape hatch traded verified token-exactness for ~0.16 MB of wire. |
