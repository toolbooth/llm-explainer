...
*《机器内部·课堂版》共享前置文档 · 草稿 2026-08-22。状态：2026-08-22 已整合进站点（构建第三阶段）；service worker 已于 2026-08-23 上线（第四阶段）；仍标注“规划中”的，是课堂版尚未完成的构建项（Chromebook 设备测试）。*
...
- **上游许可——请仔细读这一条。** 截至 2026-08-22,Hugging Face 上 `roneneldan/TinyStories-1M` 的模型卡片**没有显示任何 license 字段**。TinyStories *数据集* 卡片标注的许可是 **CDLA-Sharing-1.0**（Community Data License Agreement – Sharing）。nano-lm 的 `meta.json` 把权重记录为 “roneneldan/TinyStories (research release)”。因此本文档**不**对权重断言任何许可。再分发的依据，明说如下：该检查点由作者作为研究成果公开发布；TinyStories 数据集的许可（CDLA-Sharing-1.0 §3.5）对用该数据训练的模型不施加任何义务；文件原样再分发并注明出处。我们已在 [TinyStories-1M 讨论区 #15](https://huggingface.co/roneneldan/TinyStories-1M/discussions/15) 向上游作者正式提出明确条款的请求；若对方答复或提出异议，本分发将遵循其条款（必要时重训替代模型），并更新本节。[2026-08-22 查阅自 huggingface.co/roneneldan/TinyStories-1M 与 huggingface.co/datasets/roneneldan/TinyStories]
- **引擎许可。** nano-lm 的代码采用 MIT 许可[nano-lm README "Status";PRODUCT.md §9 第 5 行]。模型权重不在 MIT 许可范围内——见上方“上游许可”一条。
...
6. 仍在离线状态下刷新页面。**2026-08-23 已实现：** 课堂页面会从 service worker 的预缓存重新加载并继续工作（《技术检查》写了存了什么、怎么更新、怎么确认 worker 在运行）；旗舰长文没有自己的 worker，所以它扛得住课中断网，但扛不住刷新。请仍把纸质的“不插电”活动单当作备用方案，以防哪天设备带着清空的配置文件来上课[PRODUCT.md §6.1 "Offline after first load"]。
