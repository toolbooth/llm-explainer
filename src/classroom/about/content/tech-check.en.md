...
|---|---|---|---|
| Page, script and style bundles | measured at build 2026-09-15: 1.38 MB on disk (769 KB app + page shell — the front-matter documents, lesson pages, guides and the embed/evidence pages all travel inside it — + 559 KB transformers.js tokenizer library + 49 KB CSS), ~443 KB gzipped on the wire; the 23.6 MB ONNX runtime in the same build is fetched only by the flagship's optional larger model, never by a classroom page; the service worker itself is 1.9 KB | page open | yes |
...
| **Total** | **≤ 11 MB** first-visit budget, everything included [PRODUCT.md §6.1, re-baselined 2026-09-15 (r3)]. Measured 2026-09-15: 7.50 + 2.11 + 1.38 = **10.99 MB on disk**, about **8.5 MB on the wire** (the weights do not compress; the rest does). Two lines inside it are pinned separately: weights + tokenizer (9.61 MB) must stay under 10 MB (a test guards it), and the bundle line was re-baselined to its measured size — the earlier "~0.5 MB left for the bundle" figure was a pre-build guess, and replacing transformers.js with a self-contained BPE tokenizer was considered and declined (a rewrite risks drift against the token-exact-verified engine to save ~0.16 MB of wire; §6.1 r3 records the reasoning) | | |
...
- **Arithmetic.** 30 students × ~11 MB ≈ **330 MB** burst if everyone opens the page at the same second (PRODUCT.md §6.1 r3; the document's first draft computed 225 MB from the weights alone). On a 100 Mbps shared school link that is roughly 25–30 s of saturation; on a 1 Gbps link, a few seconds. Acceptable, but **not yet tested** — a 30-client simultaneous-load test is MVP item 4 [PRODUCT.md §10.1].
