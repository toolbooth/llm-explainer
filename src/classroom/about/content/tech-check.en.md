...
|---|---|---|---|
| Page, script and style bundles | measured at build 2026-08-22: 1.13 MB on disk (532 KB app + 559 KB transformers.js tokenizer library + 40 KB CSS), ~352 KB gzipped on the wire; the 23.6 MB ONNX runtime in the same build is fetched only by the flagship's optional larger model, never by a classroom page | page open | yes |
...
| **Total** | **≤ 10 MB** budget for weights + tokenizer + bundle [PRODUCT.md §6.1 r2]. Measured 2026-08-22: 7.50 + 2.11 + 1.13 = **10.74 MB on disk**, about **8.5 MB on the wire** (the weights do not compress; the rest does). The weights-plus-tokenizer line (9.61 MB) is inside the budget and pinned by a test; the bundle exceeds the ~0.5 MB PRODUCT.md left for it on disk but not on the wire — open question for the author: re-baseline the budget, or replace transformers.js with a self-contained BPE tokenizer | | |

After the first visit the browser serves all of this from cache; a **service worker** that also survives a page reload offline is a classroom-build item (planned, PRODUCT.md §6.1). On a managed Chromebook that clears site data at sign-out, every student re-downloads ~10 MB on each new session; plan bandwidth accordingly (below).
