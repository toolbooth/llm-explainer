...
| 资源 | 大小 | 时机 | 缓存？ |
|---|---|---|---|
| 页面、脚本与样式包 | 2026-09-15 构建时实测：磁盘 1.38 MB（应用加页面外壳 769 KB——前置文档、课页、指南和嵌入/证据页全部随包一起——+ transformers.js 分词器库 559 KB + CSS 49 KB），线上 gzip 后约 443 KB；同一构建里 23.6 MB 的 ONNX 运行时只有旗舰长文的可选大模型才会取，课堂页面永远不取；service worker 本身 1.9 KB | 打开页面时 | 是 |
...
| **合计** | 首次访问全部内容 **≤ 11 MB** 的预算[PRODUCT.md §6.1，2026-09-15 重定基线（r3）]。2026-09-15 实测：7.50 + 2.11 + 1.38 = **磁盘 10.99 MB**，线上约 **8.5 MB**（权重压不了，其余可以）。其中两条线各自钉住：权重加分词器（9.61 MB）必须低于 10 MB（有测试守着）；代码包一行按实测数重定了基线——之前“留给代码包约 0.5 MB”是构建前的估计，而“用自包含 BPE 分词器替换 transformers.js”的方案经权衡后放弃（重写有偏离已逐 token 验证的引擎的风险，只省约 0.16 MB 线上流量；理由记录在 §6.1 r3） | | |
...
2026-08-23 构建（PRODUCT.md §6.1“离线行为”第 (b) 级；§10.1 第 4 项）。第一次打开课堂页面时，浏览器会安装一个很小的 service worker（`/classroom-sw.js`，1.9 KB，不用任何框架），把一节课需要的全部文件存成一份**带版本号的预缓存**——页面外壳、三个脚本/样式文件、7.5 MB 权重和约 2 MB 的切词器（8 个文件，11.0 MB）。从此：
...
- **算一下。** 30 名学生 × 约 11 MB ≈ **330 MB** 的瞬时流量，如果所有人在同一秒打开页面（PRODUCT.md §6.1 r3；该文档初稿只按权重算为 225 MB）。在 100 Mbps 的共享校园出口上大约是 25–30 秒的饱和；1 Gbps 的话只要几秒。可以接受，但**尚未测试**——30 客户端并发加载测试是 MVP 第 4 项[PRODUCT.md §10.1]。
