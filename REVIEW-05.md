...
## 真模型实测（2026-09-15，TinyStories-1M fp16 权重 + 自托管 GPT-2 切词器，Node 端 nano-lm 前向；浏览器端仅抽查了 Tom-and-Lily 首轮 49 token + 续写 9 token——同一份代码与权重，Node 端全量复现，浏览器全量复现待作者过目时顺手做，见开放问题 10）
...
| 4 | — | 0.0808%（约一千二百分之一；k=1 → k=4 共 ×399，未取整值 2.02e-6 → 8.08e-4） |
...
and the sun.” 连写**三**遍作提示词，贪心下一步给出 `\n<|endoftext|>`；
两遍时它还会开始抄第三遍（下一 token 前二：“ She” 57.7% vs “\n” 38.4%）——
复读攒得越多，“收尾”越像下一步。（验收复测修正：初稿误记为两遍。）

以上全部由 `test/essay5-data.test.ts` 在每次 CI 里对着真权重重测（约 8 秒，
贪心行走 + 概率连乘逐条断言）；存活率均为从各圈起点（生成位 55/63/71）起算的整圈乘积，读者若从 token 80（第四圈中途）继续，实际存活率因自强化还要略高；`test/loops.test.ts` 用合成序列测重复检测器
本身。
